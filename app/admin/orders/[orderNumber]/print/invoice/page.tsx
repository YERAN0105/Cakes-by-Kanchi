...
import type { AddressSnapshot, ProductSnapshot } from "@/types/database";
...
export default async function InvoicePrintPage({ params }: PageProps) {
  ...
  const { data: rawOrder } = await admin
    .from("orders")
    .select(
      "*, users(name, email, phone), order_items(id, product_snapshot, customization, unit_price, line_total), coupons(code)"
    )
  ...
  const slotResult = order.time_slot_id
      ...
      <div className="bg-white rounded-2xl border border-border p-8">
        ...
        <div className="flex justify-between gap-8 mb-6">
          ...
          <div className="text-right">
            ...
            <p className="text-xs font-body uppercase tracking-wider text-ink-light mt-3 mb-1">
              Payment
            </p>
            ...
            {order.coupons?.code && (
              <p className="text-xs font-body text-wine mt-0.5">Coupon: {order.coupons.code}</p>
            )}
            {order.payment_method !== "cod" && (
              <p
                className={`text-xs font-body mt-0.5 font-medium ${
        ...
        <div className="ml-auto w-64 space-y-1.5 text-sm font-body">
          ...
          {parseFloat(order.discount_amount) > 0 && (
            <div className="flex justify-between text-wine">
              <span>
                Discount{order.coupons?.code ? ` (${order.coupons.code})` : ""}
              </span>
          ...
          {parseFloat(order.loyalty_discount) > 0 && (
            <div className="flex justify-between text-wine">
              <span>Loyalty Points ({order.loyalty_points_used} pts)</span>
              <span>−{formatCurrency(parseFloat(order.loyalty_discount))}</span>
            </div>
          )}
          {parseFloat(order.tax_amount) > 0 && (
            <div className="flex justify-between">
              <span className="text-ink-light">Tax</span>
          ...
          <div className="flex justify-between font-semibold text-base pt-2 border-t-2 border-wine">
            ...
            <span className="font-display text-wine">
        ...
        {order.notes && (
          <div className="mt-6 pt-4 border-t border-border">
            <p className="text-xs uppercase tracking-wider text-ink-light mb-1">Customer Note</p>
        ...
        <div className="mt-10 pt-5 border-t border-border text-center text-xs text-ink-light font-body">
          ...
          <p className="mt-0.5">
            Questions? Contact us at {brand.email} or {brand.phone}
          </p>
        </div>
      </div>
    </div>
